...
exports.autoAllocate = async (req, res) => {
  try {
    let { department, semester, divisions, subjects, lecturers } = req.body;
    ...
    // 1. Fetch subjects if not provided by frontend
    if (!subjects || !Array.isArray(subjects) || subjects.length === 0) {
      subjects = await Subject.find({
        department: { $regex: new RegExp(`^${department}$`, 'i') },
        semesters: parseInt(semester)
      });
      // Fallback: if no subjects found for this department+semester, fetch all subjects for this semester
    ...
    // 2. Fetch active staff if not provided by frontend
    if (!lecturers || !Array.isArray(lecturers) || lecturers.length === 0) {
      lecturers = await Staff.find({
        department: { $regex: new RegExp(`^${department}$`, 'i') },
        semesters: parseInt(semester),
        status: 'Active'
      });
      // Fallback: if no staff found for this department+semester, fetch all active staff for this semester
    ...
    // 3. Forward request to Python backend
    console.log("Forwarding auto-allocate request to Python microservice...");
    const pythonUrl = `${process.env.PYTHON_BACKEND_URL}/timetable/auto-allocate`;
    ...
    console.error("Auto allocate error:", error.message);
    if (error.response) {
      console.error("Python response error:", error.response.data);
      return res.status(error.response.status).json(error.response.data);
    }
